...
`res.app` is identical to the [req.app](/en/5x/api/request#reqapp) property in the request object.
...
[app.locals](/en/5x/api/application#applocals) instead.
...
This property holds a reference to the [request object](/en/5x/api/request)
...
Later, you may access this value through the [req.signedCookies](/en/5x/api/request#reqsignedcookies) object.
...
It uses [req.accepts()](/en/5x/api/request#reqacceptstypes) to select a handler for the request, based on the acceptable
...
[jsonp callback name](/en/5x/api/application#application-settings) setting.
