...
`res.app` is identical to the [req.app](/en/4x/api/request#reqapp) property in the request object.
...
[app.locals](/en/4x/api/application#applocals) instead.
...
This property holds a reference to the [request object](/en/4x/api/request)
...
not specified defaults to the value stated in [RFC 6265](https://datatracker.ietf.org/doc/html/rfc6265).
...
Later you may access this value through the [req.signedCookie](/en/4x/api/request#reqsignedcookies) object.
...
It uses [req.accepts()](/en/4x/api/request#reqacceptstypes) to select a handler for the request, based on the acceptable
...
[jsonp callback name](/en/4x/api/application#application-settings) setting.
...
vulnerabilities](/en/advanced/best-practice-security#prevent-open-redirects).
...
A `back` redirection redirects the request back to the [referer](https://en.wikipedia.org/wiki/HTTP_referer),
...
For more information, see [Using template engines with Express](/en/4x/guide/using-template-engines).
